...
>
> 1. Read `mytestlab123/chatgpt-aws/docs/PORTABLE_AWS_MCP_KNOWLEDGE.md`, `docs/EXPERIMENTS.md`, `docs/DRIFT_DEMO.md`, and `docs/AUTOMATION_CICD_LAB.md` first.
...
> 8. For Terraform least privilege, include provider **read/refresh APIs** as well as create/update/delete APIs; Issue #9 proved missing refresh permissions can block reconciliation.
> 9. Verify the **final effect**, not only API acceptance. For event automation, poll/read downstream evidence such as DynamoDB or service state.
> 10. AWS managed MCP requests can be governed separately with `aws:ViaAWSMCPService` / `aws:CalledViaAWSMCP`; Issue #9 contains a live S3 delete-deny proof.
> 11. After deployment, independently verify real AWS state through AWS MCP. If MCP changes a Terraform-managed resource, expect Terraform to detect/reconcile the drift.
> 12. Write durable evidence/learning back to GitHub so the next session does not depend on chat history.
>
